...
- 前端：Streamlit + 项目内本地表格组件（宽屏本地 Web 应用）
...
| 文件 | 职责 |
| --- | --- |
| `app.py` | 页面路由、会话状态、本地表格组件、筛选、表单、图表与中文 UI。 |
...
`transactions` 是唯一的业务流水表，关键字段包括：时间、账户、收支类型、金额、分类、标签、报销状态、备注、对方、支付方式与 `import_hash`。

- 收入金额在数据库中必须为正，支出金额必须为负；UI 编辑与手动录入以正数输入，再根据收支类型转换。
- 公费垫付以支出分类“公费垫付”识别；其报销状态持久化为“待报销”或“已结清”。收入分类“垫付报销”仅用于登记实际收回款项。
...
- 自动过滤的记录不得写入数据库。支付宝过滤余额宝收益发放、花呗自动还款、银行卡定时转入；微信过滤“转入零钱通”，且必须同时检查备注与映射后的分类。
...
- 所有日期时间的输入与展示必须使用 24 小时制，统一格式为 `YYYY-MM-DD HH:MM:SS`；不得出现 AM/PM 或受系统 12 小时制影响的日期时间控件。
- 流水列表使用统一的本地表格组件，支持固定表头、横向滚动、列宽调整、列顺序拖拽、键盘导航和剪贴板操作；列布局仅保存在当前浏览器。待报销公费显示淡红、已结清公费显示淡绿。
- 流水列表的全局搜索框只要存在有效关键词，就必须对整个数据库查询；当前月份和字段筛选只能继续缩小全库搜索结果，不得把搜索范围退回单月。
- 流水页及本地表格组件的高度必须随实际内容收束；表格数据较多时使用表格内部滚动，不得通过固定、累计或宿主高度在表格下方制造不必要空白。
- 整表修改必须进入本地表格组件编辑模式；单击单元格即可编辑，草稿只保存在浏览器端，编辑过程不得写库或整页重跑，只有点击“保存修改”才校验并持久化。
- 主操作使用 `type="primary"`；一个局部操作区避免出现多个无优先级的主按钮。
- 删除、覆盖、清空、恢复默认等高风险操作必须具备明确禁用条件、确认或可恢复机制。
- 修改流水界面中需要选择或确认的操作统一使用居中弹窗处理，不在表格下方单独追加横条；后续新增同类操作也遵循此规则。
